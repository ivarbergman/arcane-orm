...
class MySQL extends Bridge
{
    ...
    function cond($global_alias = false)
    {
        $str = "";
        $s = "";
        foreach ($this->condition as $c)
        {
            ...
            {
                $rh = $rh->sql($global_alias);
            }
            $str .= "{$s} ".$lh->sql($global_alias)." {$op} {$rh}";
            $s = " AnD ";
        }
    ...
    function select_with_union($e)
    {
        ...
        $sql = sprintf("SELECT %s %s FROM (  ".PHP_EOL,
                       $this->select_variant(),
                       $this->expr_list_from_select($e) );

        $sql .= implode(array_reverse($this->union), PHP_EOL.'UNION'.PHP_EOL);
        $sql .= PHP_EOL;
        $sql .= sprintf(" ) t %s %s %s %s;",
                        $this->cond(true),
                        $this->group(),
                        $this->order(),
                        $this->limit());
