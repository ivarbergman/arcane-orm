...
class BaseArcaneDB
{
  ...
  /* holds a singleton instance of the BaseArcaneDB class */
  public static $current;

  /* holds a singleton instances of database specific BaseArcaneDB class */
  ...
  public static function get($pdox = null)
  {
	...
	$bridge = new MySql();
	self::$current = new BaseArcaneDB($pdox, $bridge);
	Schema::load($pdox);
      }
    return self::$current;
  }
  ...
  public static function db($name)
  {
	...
	$bridge = new MySql();
	self::$databases[$name] = new BaseArcaneDB($pdox, $bridge);
	Schema::load($pdox);
      }
  ...
  public static function information_schema($db)
  {
    ...
    if (!isset(self::$databases[$dbname]))
      {	
	//Log::dbg('new BaseArcaneDB');
	$pdox = APdo::get($prop, $dbname);
	$bridge = new MySql();
	self::$databases[$dbname] = new BaseArcaneDB($pdox, $bridge);
      }
  ...
  public static function isolated()
  {
    $pdox = APdo::get();
    $bridge = new MySql();
    $db = new BaseArcaneDB($pdox, $bridge);
    Schema::load($pdox);
    return $db;
  }
